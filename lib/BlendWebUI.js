            ...
            if (typeof context === 'undefined') {
                if (id || this.id) {
                    ...
                    if (_type.contains(type)) {
                        context = blend.activeLayer[0];//只选active layer 的dom
                    }else{
                        context = document;
                    }
...
define(
    'src/web/api',['require','./events'],function(require) {
        ...
        api.layerStopRefresh = function(id){
            if (!id) {//默认使用active layer的id
                id = blend.activeLayer.attr("data-blend-id") || '0';
            }
            blend.get(id).endPullRefresh();
            // layerapi.endPullRefresh(blend.get(id));
        };
...
define('src/web/blend',["require",'./../common/lib',"./configs","./events",'../../third_party/jquery-2.1.1.js','./api'],
    function(require) {
        ...
        var blend = {};
        var controls = {};
        var cbs={};//临时存储，blend.start 的 layerId对应的执行函数
         ...
         * @property {Object} Api接口
         */
        blend.start = function(layerId,callback){
            if (layerId == '0') {
    ...
    'src/web/layer/layerapi',['require'],function(require) {
        ...
        api.prepare = function(id, options, context, dom) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // context.state = '';
                    // options.onsuccess && options.onsuccess(data); //move to paint
        ...
        api.startPullRefresh = function(context) {
            ...
            var afterRefresh = function(container) {
                ...
                $(".pull-to-refresh-label",tipLayer).text("加载中...");//todo: 让这个可自定义
            ...
            pullEvents.handleTouchMove = function(e) {
                ...
                if (touchesDiff > 0 && container[0].scrollTop <= 0 || container[0].scrollTop < 0) {
                    ...
                    if ((useTranslate && Math.pow(touchesDiff, 0.85) > 44) || (!useTranslate && touchesDiff >= 88)) {
                        ...
                        $(".pull-to-refresh-label",tipLayer).text("释放刷新...");
                    }
                    else {
                        refresh = false;
                        tipLayer.removeClass('pull-up');
                        $(".pull-to-refresh-label",tipLayer).text("下拉刷新...");
                    }
                }
                else {
                    tipLayer.removeClass('pull-up');
            ...
            pullEvents.handleTouchEnd = function(e) {
                ...
                if (refresh) {
                    ...
                    $(".pull-to-refresh-label",tipLayer).text("加载中...");
        ...
        */
        api.startSwipe = function(context, container) {
            ...
            pageEvents.handleTouchStart = function(e) {
                if (!allowViewTouchMove || isTouched) return;
                isMoved = false;
                isTouched = true;
                isScrolling = undefined;
                touchesStart.x = e.type === 'touchstart' ? e.targetTouches[0].pageX : e.pageX;
            ...
            pageEvents.handleTouchEnd = function(e) {
                ...
                if (pageChanged) {
                    ...
                    if (previousContext.myGroup) {
                        // previousContext.myGroup.fire('beforeshow');
                    }
                ...
                context.transitionEnd(function () {
                    ...
                    if (pageChanged) {
                        ...
                        if (previousContext.myGroup) {
                            // previousContext.myGroup.fire('onshow');
                        }
                    }
                });
            };
        ...
        Layer.prototype._initEvent = function  () {
            ...
            me.on('onshow',function(event){
                ...
                if (me.myGroup) {
                    ...
                    // console.log("111111111111111  "+event.detail)
                }

            });

            //me.on('layerCreateSuccess',me.onrender);
            //after render
            me.on('onrender',me.onrender);
        ...
        Layer.prototype.in = function(){
            ...
            */
            ...
            if(this.state) {
                ...
                console.warn(" cant' move in. it has state..." + this.state);
                return ;
            }
            // state
            this.state = "slide";
            ...
            if (!this.myGroup || this.myGroup.isActive()){
                ...
                layerinobj = this.myGroup;//SEE IT
                if (layerinobj.activeId !== this.id) {//layergoup的activeid 需要变化
                    //管理 activeid 放在了layer group里面
                    $(layerinobj.__layers[layerinobj.activeId].main).addClass("page-on-right").removeClass('page-on-center');
                    $(layerinobj.__layers[this.id].main).removeClass("page-on-right").addClass('page-on-center');
            ...
            if ( lowerAnimate ) {
                if ( translationReverse ) {
                    ...
                    layerin.removeClass('page-on-right').addClass('page-on-center');
                }else{
                    layerout.removeClass("page-on-center").addClass("page-on-left");
                    layerin.removeClass('page-on-right').addClass('page-on-center');
                }
                ...
                if ( translationReverse ) {
                    ...
                    //2. 滑入新page
                    layerin.removeClass('page-on-right').addClass('page-on-left').addClass('page-from-left-to-center');
                }else{
                    //标准的页面进入流程
                    //1. 找到当前page，然后动画走掉
                    layerout.removeClass("page-on-center").addClass("page-from-center-to-left");
                    //2. 滑入新page
                    layerin.removeClass('page-on-left').addClass('page-on-right').addClass('page-from-right-to-center');
                }
            }
            ...
            var afteranimate = function(){
                ...
                // if ( me.myGroup ) {
                //     // me.myGroup.fire("onshow");
                // }
             
            };
            //入场动画结束
            if (!lowerAnimate){
                layerinobj.animationEnd(function(){

                    if (translationReverse){
                        layerout.removeClass(function(index,css){
                            return (css.match (/\bpage-\S+/g) || []).join(' ');
                            // return css.replace(/\bpage-\S+/g, '');
                        }).addClass("page page-on-right");
                        layerin.removeClass().addClass('page page-on-center');
                    }else{
                        layerout.removeClass().addClass("page page-on-left");
